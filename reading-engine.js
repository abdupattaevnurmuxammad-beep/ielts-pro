...
// Official Academic Reading raw-score → band conversion (out of 40).
...
function getBand(rawScoreOutOf40) {
  for (const row of READING_BAND_TABLE) {
    if (rawScoreOutOf40 >= row.min) return row.band;
  }
  return 1;
}
...
function showResults(correct, total, band, results) {
  ...
  let html = `
    ...
    <div class="feedback-row"><strong>Estimated Band:</strong> ${band} </div>
  `;
